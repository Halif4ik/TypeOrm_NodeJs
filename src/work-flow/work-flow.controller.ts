...
import {AdditionalUpdateQuizId, GetRedisAllQuizDto, GetRedisQuizDto,} from "../quizz/dto/update-quizz.dto";
import {FileResponse, TAnswers, TPassedQuiz,} from "../GeneralResponse/interface/customResponces";
...
export class WorkFlowController {
   ...
   async exportQuiz(@UserDec() userFromGuard: User, @Query() getRedisQuizDto: GetRedisQuizDto,
      ...
      const fileResponse: FileResponse = await this.workFlowService.exportQuizDataFromRedis(
          userFromGuard,
          getRedisQuizDto,
      );
   ...
   async exportUser(@UserDec() userFromGuard: User, @Query() getRedisAllQuizDto: GetRedisAllQuizDto,
      ...
      const fileResponse: FileResponse =
          await this.workFlowService.exportUserDataFromRedis(
              userFromGuard,
              getRedisAllQuizDto,
          );
      res.set(fileResponse.header);
